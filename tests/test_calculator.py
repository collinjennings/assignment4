...
def test_display_history_with_entries(capsys):
    ...
    history = [
        ...
        "ModulusCalculation: 10.0 Modulus 3.0 = 1.0",
...
5. ModulusCalculation: 10.0 Modulus 3.0 = 1.0
6. PowerCalculation: 2.0 Power 3.0 = 8.0"""
    assert captured.out.strip() == expected_output.strip()
...
def test_calculator_power_operation(monkeypatch, capsys):
    """
...
def test_calculator_modulus_operation(monkeypatch, capsys):
    """
...
def test_calculator_modulus_by_zero(monkeypatch, capsys):
    ...
    - Act: Call the calculator function.
    - Assert: Verify that a modulus by zero error message is displayed.
    """
    # Arrange
    ...
    # Assert
    captured = capsys.readouterr()
    assert "Cannot divide by zero." in captured.out    #def test_calculator_division_by_zero(monkeypatch, capsys):
    """
    Test the calculator's handling of division by zero.
    ...
    # Assert
    captured = capsys.readouterr()
    assert "Cannot divide by zero." in captured.out #pragma: no cover
...
def test_calculator_unsupported_operation(monkeypatch, capsys):
    ...
    """
    # Arrange
    user_input = 'square 2 2\nexit\n'  # Changed 'modulus to 'square' to ensure unsupported operation
    monkeypatch.setattr('sys.stdin', StringIO(user_input))
    ...
    # Assert
    captured = capsys.readouterr()
    assert "Unsupported calculation type: 'square'." in captured.out
